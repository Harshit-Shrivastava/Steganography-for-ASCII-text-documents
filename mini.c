...
char password[16];
char secret[max];
//char secret[]="1001011010111001"; //msg to be embedded into the cover.txt file
//int secretlen=sizeof(secret)/sizeof(secret[0])-1; //number of bits in the secret message
int secretlen=0;
...
int input(){
	...
	FILE* f1=fopen("secret.txt","r"); //secret text
...
char c,buf[64],pos=0;
	...
	if(c!=EOF) putc(c,f2); //to include spaces, tabs or new lines in cover.txt
    c=getc(f1);
...
/*
if(argc!=2){
printf("%s","User password required\n");
exit(1);
}
*/
